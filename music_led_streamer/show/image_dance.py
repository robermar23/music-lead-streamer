...
# Image settings
IMAGE_PATH = "images/306667.png"  # image to load (must be in the same directory)
NUM_ROWS = 30  # max Number of rows to split the image, if screen is too small, it will be less
NUM_COLS = 30 # max Number of columns to split the image, if screen is too small, it will be less
...
def audio_callback(indata, frames, time, status):
    global volume, bass, midrange, treble, smoothed_bass, smoothed_treble, smoothed_midrange
    if status:
...
def extract_top_colors(image, num_colors=3):
    ...
    top_colors = [color for color, _ in counter.most_common(num_colors)]
    return top_colors
